...
from .bkp_observer import BackupObserver, Backup
...
def intersect_files(local_files: dict[str, Backup], remote_files: [ResourceObject]) -> [BackupFile]:
    ...
    for backup in local_files.values():
        ...
        result_file = BackupFile(backup.slug, backup.name, backup.date, True)
    ...
    for remote_file in remote_files:
        if remote_file.name in remote_file_processed:
            continue
        result_file = BackupFile('', remote_file.name, str(remote_file.modified), False, True)
        result.append(result_file)
...
class YaDsk:
    ...
    def create_token(self, check_code):
        self._LOGGER.info('Try get token')
        try:
            self._LOGGER.debug("Try get token")
